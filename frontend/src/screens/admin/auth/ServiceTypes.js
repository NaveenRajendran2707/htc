...
import useServiceTypesHook from "../../../api/serviceTypes";
import {
  Spinner,
  ViewServiceTypes,
  Pagination,
...
const ServiceTypes = () => {
  ...
  const { data, isLoading, isError, error, refetch } = getServiceTypes;
  ...
  const submitHandler = (data) => {
    edit
      ? mutateAsyncUpdate({
          _id: id,
          serviceSerialNo: data.serviceSerialNo,
  ...
  const viewHandler = (serviceType) => {
    setId(serviceType._id);
    setView(true);
    setValue("serviceSerialNo", serviceType.serviceSerialNo);
    setValue("serviceName", serviceType.serviceName);
    setValue("particular", serviceType.particular);
    setValue("fees", serviceType.fees);
    setValue("status", serviceType.status);
  };

  const editHandler = (serviceType) => {
    setId(serviceType._id);
    setView(false);
    setEdit(true);
    setValue("serviceSerialNo", serviceType.serviceSerialNo);
  ...
  return (
    <>
      <Helmet>
        <title>Service Types | HTC</title>
        <meta property="og:title" content="Service Types" key="title" />
      </Helmet>
      {isSuccessDelete && (
      ...
      {isSuccessPost && (
        <Message variant="success">
          Service Type has been Created successfully.
        </Message>
      )}
          ...
          <DialogPanel className="w-full max-w-[800px] max-h-[calc(100vh_-_1rem)] overflow-auto flex flex-col rounded-xl shadow-sm bg-white">
            ...
            >
              <h3 className="text-2xl font-bold">
                {edit ? "Edit ServiceType" : view ? "View ServiceType" : "Add ServiceType"}
              </h3>
