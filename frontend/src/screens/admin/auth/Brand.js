...
import useBrandHook from "../../../api/brand";
import {
  Spinner,
  ViewBrand,
  Pagination,
...
const Brand = () => {
  ...
  const { data, isLoading, isError, error, refetch } = getBrands;
  ...
  return (
    <>
      <Helmet>
        <title>Brand | HTC</title>
        <meta property="og:title" content="Brand" key="title" />
      </Helmet>
      {isSuccessDelete && (
          ...
          <DialogPanel className="w-full max-w-[800px] max-h-[calc(100vh_-_1rem)] overflow-auto flex flex-col rounded-xl shadow-sm bg-white">
            ...
            >
              <h3 className="text-2xl font-bold">
                {edit ? "Edit Brand" : view ? "View Brand" : "Add Brand"}
              </h3>
