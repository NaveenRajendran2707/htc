...
const schemaName = ItemGroup;
const schemaNameString = "ItemGroup";
...
export const postItemGroup = async (req, res) => {
  try {
...
export const putItemGroup = async (req, res) => {
  try {
    const { id } = req.params;
    const { itemGroupSerialNo, itemGroup, itemSubGroup } = req.body;

    const object = await schemaName.findById(id);
    if (!object)
      return res.status(400).json({ error: `${schemaNameString} not found` });

    object.itemGroupSerialNo = itemGroupSerialNo;
    object.itemGroup = itemGroup;
    object.itemSubGroup = itemSubGroup;
